...
import jaxmarl
import networks.human as human
import os
import orbax
...
# Add the ActorCritic class from your training script
...
def step_human(obs, rng):
    global human_params
    # obs has shape (num_envs, *obs_shape) 
    # We need to apply the network to each environment separately
    # Flatten each observation in the batch
    if len(obs.shape) > 2:
        # obs is (num_envs, *spatial_dims) -> flatten spatial dims
        obs_flat = obs.reshape(obs.shape[0], -1)
    else:
        obs_flat = obs
    
    # Apply network to each observation in batch using vmap
    def apply_fn(single_obs):
        pi, val = human_network.apply(human_params, single_obs)
        return pi, val
    
    pi_batch, val_batch = jax.vmap(apply_fn)(obs_flat)
    
    # Sample actions for each environment
    rngs = jax.random.split(rng, obs_flat.shape[0])
    actions = jax.vmap(lambda pi, r: pi.sample(seed=r))(pi_batch, rngs)
...
if __name__ == "__main__":
    ...
    # USE THE SIMPLE ACTORCRITIC INSTEAD OF CONV NETWORK
    human_network = ActorCritic(env.action_space().n, activation="tanh")
    
    # Initialize with flattened observation
    init_x = jnp.zeros(env.observation_space().shape).flatten()
    initial_human_params = human_network.init(rng, init_x)

    orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
    if args.checkpoint is None:
        checkpoint = os.path.join(os.getcwd(), f"cramped_room/self_play")
    else:
    ...
    # Extract params from the checkpoint
    # The checkpoint structure is: raw_restored["model"]["params"]
    human_params_raw = raw_restored["model"]["params"]
    
    # Print checkpoint structure for debugging
    print("Checkpoint structure:")
    print(f"Params structure: {jax.tree_util.tree_map(lambda x: x.shape, human_params_raw)}")
    
    # Remove the extra batch dimension that comes from vmap training
    # The shapes are (1, 520, 64) but should be (520, 64)
    human_params = jax.tree_util.tree_map(
        lambda x: x.squeeze(0) if len(x.shape) > 1 and x.shape[0] == 1 else x,
        human_params_raw
    )
    
    print("After removing batch dimension:")
    print(f"Params structure: {jax.tree_util.tree_map(lambda x: x.shape, human_params)}")
    
    # Verify the first layer kernel has the right shape now
    first_layer_kernel = human_params['params']['Dense_0']['kernel']
    checkpoint_obs_dim = first_layer_kernel.shape[0]
    expected_obs_dim = reduce(lambda x, y: x * y, env.observation_space().shape, 1)
    
    print(f"Checkpoint obs_dim: {checkpoint_obs_dim}")
    print(f"Environment obs_dim: {expected_obs_dim}")
    
    if checkpoint_obs_dim != expected_obs_dim:
        raise ValueError(f"Observation dimension mismatch: checkpoint={checkpoint_obs_dim}, env={expected_obs_dim}")
    ...
    config["version"] = 1#version.__version__
