...
class EmpowermentState(NamedTuple):
    ...
    emp_step: int
    rng: jax.random.PRNGKey  # Add RNG to state
...
# UTILITY FUNCTIONS
# ============================================================================
def get_rollout(train_state, config, pretrained_params=None):
    ...
    while not done:
        ...
        # Agent 1 uses pretrained policy if available, otherwise same as agent 0
        if pretrained_params is not None:
            pi_1, _ = network.apply(pretrained_params, obs["agent_1"])
        else:
...
def render_state_seq(state_seq, env):
    ...
    frame_seq = np.moveaxis(frame_seq, -1, 1)  # Move channel axis
...
def make_train(config, pretrained_params=None):
    ...
    def train(rng):
        ...
        if config.get("use_empowerment", False):
            ...
            # Initialize RNG for empowerment
            ...
            emp_state = EmpowermentState(
                ...
                emp_step=0,
                rng=emp_rng  # Store RNG in state
            )
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                train_state, env_state, last_obs, rng, emp_state = runner_state
                ...
                # Use pretrained agent for agent_1 if specified
                if pretrained_params is not None:
                    # Agent 0 is being trained
                    pi_0, value_0 = network.apply(train_state.params, obs_batch[0::2])
                    # Agent 1 uses pretrained weights
                    pi_1, value_1 = network.apply(pretrained_params, obs_batch[1::2])
                    
                    # Interleave the actions and values
                    ...
                    # Normal case: both agents being trained
                ...
                # ====================================================================
                # ADD EMPOWERMENT REWARD TO AGENT_0 (if enabled)
                # ====================================================================
                if config.get("use_empowerment", False) and emp_state is not None:
                    # Get agent_0 and agent_1 observations and actions
                    # obs_batch is (NUM_ACTORS, obs_dim), where even indices are agent_0
                    agent_0_obs_flat = obs_batch[0::2]  # (num_envs, obs_dim)
                    agent_0_action_flat = action[0::2]  # (num_envs,)
                    agent_1_action_flat = action[1::2]  # (num_envs,)
                    
                    # Compute empowerment reward for agent_0
                    # Use next observation as goal
                    next_obs_batch = batchify(obsv, env.agents, config["NUM_ACTORS"])
                    agent_0_next_obs = next_obs_batch[0::2]  # (num_envs, obs_dim)
                    ...
                    emp_reward = compute_empowerment_reward(
                        ...
                        agent_0_action_flat,
                        agent_1_action_flat,
                        agent_0_next_obs,  # Use next state as goal
                        config["reward_type"],
                        config["repr_dim"]
                    )
                    
                    # Add empowerment reward to agent_0
                    # reward["agent_0"] is shape (num_envs,)
                    # Scale empowerment reward
                    emp_scale = config.get("emp_reward_scale", 1.0)
                    reward["agent_0"] = emp_reward #reward["agent_0"] + emp_reward * emp_scale
                # ====================================================================
                ...
                runner_state = (train_state, env_state, obsv, rng, emp_state)
                return runner_state, transition

            runner_state, traj_batch = jax.lax.scan(_env_step, runner_state, None, config["num_steps"])

            # UPDATE EMPOWERMENT BUFFER (if enabled)
            train_state, env_state, last_obs, rng, emp_state = runner_state
            
            if config.get("use_empowerment", False) and emp_state is not None:
                # Extract agent_0 data for empowerment learning
                agent_0_obs = traj_batch.obs[0::2]  # Even indices
            ...
            if pretrained_params is not None:
                _, last_val_0 = network.apply(train_state.params, last_obs_batch[0::2])
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        # RERUN NETWORK
                        if pretrained_params is not None:
                            # Only train on agent_0's experiences
                            ...
                            # Disable entropy regularization when using empowerment
                            ent_coef = 0.0 if config.get("use_empowerment", False) else config["ent_coef"]
                        else:
                ...
                train_state, traj_batch, advantages, targets, rng, emp_state = update_state
                rng, _rng = jax.random.split(rng)
                ...
                update_state = (train_state, traj_batch, advantages, targets, rng, emp_state)
            ...
            rng = update_state[4]
            emp_state = update_state[5]

            runner_state = (train_state, env_state, last_obs, rng, emp_state)
            return runner_state, metric

        rng, _rng = jax.random.split(rng)
        runner_state = (train_state, env_state, obsv, _rng, emp_state)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_agent_path", type=str, default=None, 
                        help="Path to pretrained agent checkpoint (agent_1 will use this, agent_0 will be trained)")
    ...
    parser.add_argument("--repr_buffer_size", type=int, default=80000,
    ...
    args.save=True
    #args.pretrained_agent_path = os.path.join(os.getcwd(), f"cramped_room/self_play")
    args.use_empowerment = False #True
    ...
    if args.pretrained_agent_path:
        group_name = "pretrained_" + args.layout
    ...
    if config.get("pretrained_agent_path") is not None:
        ...
        try:
            # Try: checkpoint["model"]["params"]
            raw_params = pretrained_ckpt["model"]["params"]
        except (KeyError, TypeError):
            try:
                # Try: checkpoint["model"][0].params
        ...
        # Function to remove batch dimension (first dimension) from all params
    ...
    with jax.disable_jit(False):
        ...
        '''
        returns = np.array(out["metrics"]["returned_episode_returns"][0])
        np.savetxt(
                f"rewards_{args.layout}_{args.output}.txt",
                returns,
                fmt='%.4f'
                )
        '''
        ...
        # Generate and log final rollout video
        print("Generating final rollout video...")
        env = jaxmarl.make(config["env_name"], **config["env_kwargs"])
        # runner_state is a tuple: (train_state, env_state, last_obs, rng, emp_state)
        # We vmapped over seeds, so index [0] gets the first seed
        final_train_state = out["runner_state"][0]  # This is the full tuple for first seed
        # Extract just the train_state (first element of the tuple)
        train_state_only = final_train_state[0] if isinstance(final_train_state, tuple) else final_train_state
        
        # Remove batch dimension from params (vmap adds it)
        def squeeze_batch_dim(params):
        ...
        # Create a new train_state with squeezed params
        train_state_no_batch = train_state_only.replace(
            params=squeeze_batch_dim(train_state_only.params)
        )
        
        state_seq = get_rollout(train_state_no_batch, config, pretrained_params)
